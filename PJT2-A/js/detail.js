...
let videoDetailHtml1 = "";

videoDetailHtml1 +=
`
<div class="card-body">
<h5 class="card-title">${videoList[0].title}</h5>
<iframe class="card-img-top" width="700" height="800" src="https://www.youtube.com/embed/${videoList[0].id}" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>
<p class="card-text">${videoList[0].channelName}</p>
<p class="card-text">${videoList[0].date}<small class="text-body-secondary">Last updated 3 mins ago</small></p>
</div>
`

document.querySelector("#detail1").innerHTML = videoDetailHtml1


const reviewForm = document.querySelector("#reviewForm");
const commentsContainer = document.querySelector("#reviews");
reviewForm.addEventListener("submit", function (e) {
  e.preventDefault();
  const usernameInput = reviewForm.elements.username;
  const commentInput = reviewForm.elements.comment;
...
const addComment = (username, comment) => {
    if(username == "" |comment == ""){
        window.alert("please input your username and contents")
    }else{
