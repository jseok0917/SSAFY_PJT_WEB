...
//	}
	...
	public List<Video> searchVideo(String keyword) {
	    ...
	    int[] failure = getFailureFunction(keyword);
	    ...
	    for (Video video : videoList) {
	        ...
	        for (int i = 0; i < title.length(); i++) {
	            while (j > 0 && title.charAt(i) != keyword.charAt(j)) {
	                j = failure[j - 1];
	            }
	            if (title.charAt(i) == keyword.charAt(j)) {
	                if (j == keyword.length() - 1) {
	                    matchedVideos.add(video);
	                    break;  // 해당 비디오는 이미 매칭되었으므로 다음 비디오로 넘어갑니다.
	                } else {
	                    j++;
	                }
	            }
